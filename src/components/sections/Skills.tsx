...
export default function Skills() {
  ...
  return (
    <section id="skills" ref={sectionRef} className="relative py-28 px-6 overflow-hidden">
      {/* Background glow */}
      <div className="absolute top-1/2 right-0 w-[400px] h-[400px] bg-[#00fedf]/5 rounded-full blur-[120px] pointer-events-none" />

      <div className="max-w-[1200px] mx-auto">
        {/* Section Label */}
        <div className="reveal opacity-0 translate-y-6 transition-all duration-700 flex items-center gap-3 mb-4">
          <span className="text-[#00fedf] font-mono text-xs uppercase tracking-[0.2em]">// 02</span>
          <span className="text-[#00fedf] font-mono text-xs uppercase tracking-[0.2em]">Technical Arsenal</span>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-4">
          {skills.map((skill, index) => (
            ...
            >
              <div className="flex flex-col items-center justify-center gap-3 p-4 bg-[#131318] border border-[#3a4a46] rounded-xl hover:border-[#00fedf]/50 hover:bg-[#1f1f25] neon-glow-cyan-hover transition-all duration-300 cursor-default h-24">
                ...
                />
                <span className="text-[#b9cac5] font-mono text-xs group-hover:text-[#00fedf] transition-colors duration-300 text-center leading-tight">
                  {skill.name}
                </span>
              </div>
            </div>
